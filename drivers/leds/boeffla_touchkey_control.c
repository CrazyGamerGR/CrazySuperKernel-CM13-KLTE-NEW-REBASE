...
int btkc_mode    = MODE_TOUCHKEY_ONLY;		// touchkey only mode is default in S5 Samsung Stock
...
{
	if (touch_led != NULL)
		if (touch_led->enabled)
			i2c_smbus_write_byte_data(touch_led->client, CYPRESS_GEN, TK_CMD_LED_ON);
}
...
static void touchkey_led_disable(void)
{
	if (touch_led != NULL)
		if (touch_led->enabled)
			i2c_smbus_write_byte_data(touch_led->client, CYPRESS_GEN, TK_CMD_LED_OFF);
}
...
void btkc_touch_start()
{
	...
	// do nothing when touchkey_only or off mode 
	// or if screen off (to not interfere with double touch to wake)
	if (btkc_mode != MODE_NORMAL)
		return;

	// enable touch key led
	touchkey_led_enable();
}
...
void btkc_touch()
{
	...
	// no special handling when touchkey_only or off mode
	// or if screen off (to not interfere with double touch to wake)
	if (btkc_mode != MODE_NORMAL)
		return;
...
{
	pr_debug("Boeffla touch key control: Led sysfs hook - status %d\n", state);

	// touchkey only mode = never block the LED
	if (btkc_mode == MODE_TOUCHKEY_ONLY)
		return false;

	// for all other modes = block the LED
	return true;
}
